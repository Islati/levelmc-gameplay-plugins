...
public class Raycast {
    ...
    private Entity hurtEntity = null;

    private Block hurtBlock = null;
    ...
    @Getter
    @Setter
    private int distanceBetween = 2;

    private int skippedRenders = 0;
    ...
    @Getter
    @Setter
    private ParticleRenderer particleRenderer = new DefaultRenderer().particle(Particle.CLOUD);
    ...
    protected void handleRaycastRendering(Location loc) {
        if (!this.showRayCast) {
            return;
        }

        if (distanceBetween > 0 && skippedRenders < distanceBetween) {
            skippedRenders += 1;
            return;
        }
        particleRenderer.render(loc);
        skippedRenders = 0;
    }
    ...
    public boolean compute(RaycastType rayCastType) {
        ...
        if (rayCastType == RaycastType.BLOCK) {
            ...
            while (this.passthroughMaterials.contains(this.rayCastLocation.getBlock().getType()) && length <= this.size * 100.0D) {
                ...
                length++;
                computeLocation(new Vector(0.0D, 0.0D, length + 50.0D));
                handleRaycastRendering(this.rayCastLocation);
            }
            ...
            Collection<Entity> entities = this.world.getNearbyEntities(this.rayCastLocation, getHitboxSize(), getHitboxSize(), getHitboxSize());
            while ((entities.size() <= 0 || entities.contains(this.owner)) && length <= this.size * 100.0D) {
                this.testedLocations.add(this.rayCastLocation);
                length++;
                computeLocation(new Vector(0.0D, 0.0D, length + 50.0D));
                entities = this.world.getNearbyEntities(this.rayCastLocation, getHitboxSize(), getHitboxSize(), getHitboxSize());
                handleRaycastRendering(this.rayCastLocation);
            }
            if (entities.size() > 0) {
    ...
    private void computeLocation(Vector rayCastPos) {
        rayCastPos = RaycastMathUtil.rotate(rayCastPos, this.yaw, this.pitch);
    ...
    public boolean hasEntity() {
        return hurtEntity != null;
    }

    public boolean hasBlock() {
        return hurtBlock != null;
    }
}