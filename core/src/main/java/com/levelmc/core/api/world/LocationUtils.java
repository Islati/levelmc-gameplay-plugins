...
public class LocationUtils {
    ...
    public static Set<Player> getPlayersInRadius(Location location, double radius) {
        ...
        Collection<Player> onlinePlayers = PlayerUtils.allPlayers();
    ...
    public static List<Location> getCubeFilled(Location center, int radius) {
        if (radius < 0) {
            return new ArrayList<Location>(0);
        }
        int iterations = (radius * 2) + 1;
        List<Location> blocks = new ArrayList<Location>(iterations * iterations * iterations);
        for (int x = -radius; x <= radius; x++) {
    ...
    public static List<Location> getCubeOutline(Location corner1, Location corner2, double particleDistance) {
